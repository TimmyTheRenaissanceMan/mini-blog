...
def register():
    ...
    if current_user.is_authenticated:
        ...
        if request.method == "GET":
            ...
            user = User.query.filter_by(user=request.form["email"]).first()
            if user:
                flash("This email has already been registered")
                return render_template("register.html", form=form)
            else:
                new_user = User(user=request.form["email"],
                                password=generate_password_hash(request.form["password"], "pbkdf2:sha512", 8),
...
def show_post(post_id):
    ...
    if request.method == "GET":
        ...
        new_comment = Comment(text=request.form["text"], post_id=post_id)
